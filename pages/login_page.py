...
class LoginPage(BasePage):
    ...
    def should_be_register_form(self):
        assert self.is_element_present(*LoginPageLocators.REGISTER_EMAIL), "Reg email is not presented"
        assert self.is_element_present(*LoginPageLocators.REGISTER_PASSWORD_1), "Reg password_1 is not presented"
        assert self.is_element_present(*LoginPageLocators.REGISTER_PASSWORD_2), "Reg repeat password_2 is not presented"
        assert self.is_element_present(*LoginPageLocators.REGISTER_BUTTON), "Reg button is not presented"

    def register_new_user(self, email, password):
        register_email = self.browser.find_element(*LoginPageLocators.REGISTER_EMAIL)
        register_email.send_keys(email)
        register_pass1 = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD_1)
        register_pass1.send_keys(password)
        register_pass2 = self.browser.find_element(*LoginPageLocators.REGISTER_PASSWORD_2)
